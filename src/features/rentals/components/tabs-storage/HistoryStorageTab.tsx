...
export const HistoryStorageTab = () => {
  const { data: spaces, isLoading, error } = useStorageSpaceListingsByType('history');
  ...
  const columns = [
    ...
    { key: "actions", label: "", className: "text-right whitespace-nowrap", hideOnMobile: true, render: (s: StorageSpace) => <StorageRowActions storageSpace={s} tab="historik" /> },
  ];
  ...
  const mobileCardRenderer = (s: StorageSpace) => (
    <div className="space-y-2">
      ...
      <StorageRowActions storageSpace={s} tab="historik" variant="mobile" />
    </div>
  );
  ...
  return (
    <div className="flex flex-col space-y-4">
      ...
      <ResponsiveTable
        ...
        rowClassName="group"
        onRowClick={(s) => navigate(`/rentals/storage/${s.id}`, { state: { from: "?tab=historik" } })}
      />
