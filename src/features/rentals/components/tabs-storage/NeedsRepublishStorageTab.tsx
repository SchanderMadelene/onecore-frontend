...
export const NeedsRepublishStorageTab = () => {
  const { data: spaces, isLoading, error } = useStorageSpaceListingsByType('needs-republish');
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-[200px]">
        ...
        <span>Hämtar förråd som behöver publiceras...</span>
      </div>
    );
  }
      ...
      <div className="flex flex-col space-y-4">
        ...
        <div className="flex items-center justify-center h-[200px] text-muted-foreground border rounded-md">
          <div className="text-center">
            ...
            <p>Inga förråd behöver publiceras</p>
          </div>
        </div>
      </div>
    );
  ...
  const columns = [
    ...
    { key: "actions", label: "", className: "text-right whitespace-nowrap", hideOnMobile: true, render: (s: StorageSpace) => <StorageRowActions storageSpace={s} tab="behovAvPublicering" /> },
  ];
  ...
  const mobileCardRenderer = (s: StorageSpace) => (
    <div className="space-y-2">
      ...
      <StorageRowActions storageSpace={s} tab="behovAvPublicering" variant="mobile" />
    </div>
  );
  ...
  return (
    <div className="flex flex-col space-y-4">
      ...
      <ResponsiveTable
        ...
        rowClassName="group"
        onRowClick={(s) => navigate(`/rentals/storage/${s.id}`, { state: { from: "?tab=behovAvPublicering" } })}
      />
