...
export const OfferedStorageTab = () => {
  const { data: spaces, isLoading, error } = useStorageSpaceListingsByType('offered');
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-[200px]">
        ...
        <span>Hämtar erbjudna förråd...</span>
      </div>
    );
  }
  ...
  const columns = [
    ...
    { key: "expiresAt", label: "Sista svarsdatum", className: "whitespace-nowrap", hideOnMobile: true, render: (s: StorageSpace) => s.offer?.expiresAt || "" },
    { key: "actions", label: "", className: "text-right whitespace-nowrap", hideOnMobile: true, render: (s: StorageSpace) => <StorageRowActions storageSpace={s} tab="erbjudna" /> },
  ];
  ...
  const mobileCardRenderer = (s: StorageSpace) => (
    <div className="space-y-2">
      ...
      <StorageRowActions storageSpace={s} tab="erbjudna" variant="mobile" />
    </div>
  );
  ...
  return (
    <div className="flex flex-col space-y-4">
      ...
      <ResponsiveTable
        ...
        rowClassName="group"
        onRowClick={(s) => navigate(`/rentals/storage/${s.id}`, { state: { from: "?tab=erbjudna" } })}
      />
